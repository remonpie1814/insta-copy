...
const InputPH = ({ placeholder, setValue, ispwd: isPwd }) => {
  const [isShow, setShow] = useState(false);
  const [active, setActive] = useState(false);

  return (
    <div
      className="relative bg-gray-100 w-[270px] h-[40px] mx-auto 
      text-sm rounded-sm border-[1px] border-gray-300 focus-within:border-gray-500"
    >
      <input
      ...
      <label
        htmlFor="myInput"
        className={` text-gray-400 pl-3.5 w-full 
        text-left pointer-events-none absolute top-1/2 left-1/2
        transform origin-left -translate-x-1/2 -translate-y-1/2 transition-all duration-200 
